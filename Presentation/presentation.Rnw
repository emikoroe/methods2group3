...
<<warning=FALSE, echo=FALSE, tidy=TRUE>>=
par(mfrow=c(2,1))
plot(model, which = 5)
cutoff <- 12/((nrow(body) - length(model$coefficients) - 2))
plot(model, which =4, cooks.levels = cutoff)
@
\end{column}
\end{columns}
\end{frame}
...
\begin{tabular}{cccc}
  ...
  0.05 & 0.996 & 0.838 & 0.915\\
  0.10 & 0.992 & 0.892 & 0.941\\
  0.15 & 0.988 & 0.942 & 0.964\\
  ... & ...    & ... & ...\\
  0.55 & 0.968 & 0.981 & 0.974\\
  ... & ... & ... & ...\\
  0.85 & 0.923 & 0.992 & 0.959\\
  0.90 & 0.903 & 0.992 & 0.949\\
   \hline
\end{tabular}
}
\end{table}
